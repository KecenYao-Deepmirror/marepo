...
#     --transformer_json ../transformer/config/nerf_focal_12T1R_256_homo.json --load_scheme2_sc_map True --save_result True\
...
for scene in ${DATASET_PATH_TEST}; do
  ...
  then
    ...
    for test_batch_size in "${test_batch_sizes[@]}"; do
      ...
      OMP_NUM_THREADS=12 CUDA_VISIBLE_DEVICES=0,1 python $testing_exe "${scene}" "$marepo_head_path" --head_network_path ${ace_head_path} \
      --transformer_json ../transformer/config/nerf_focal_12T1R_256_homo.json --load_scheme2_sc_map True --test_batch_size ${test_batch_size} --save_result True --session "b${test_batch_size}"\
